...
def classify_difficulty(board):
    ...
    level = "extreme"
    for threshold, diff_level in sorted(difficulty_levels.items(), reverse=True):
        if filled >= threshold:
            level = diff_level
            break
    ...
    bucket = "unknown"
    for b_name, levels_in_bucket in difficulty_buckets.items():
        if level in levels_in_bucket:
            bucket = b_name
            break
